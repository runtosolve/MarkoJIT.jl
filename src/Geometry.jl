...
@with_kw struct JoistDimensions
    ...
    x_support_to_bottom_chord::Float64 
...
function define_first_chord_hole_location(;joist_end_length)
	...
	if joist_end_length == 12.0

		Δx_chord_hole_from_joist_end = 2.76

	elseif joist_end_length == 24.0
...
function calculate_joist_diagonal_coordinates(span_length, depth, x_support_to_bottom_chord, node_spacing, bearing_seat, chord)
    ...
    if joist_ends[1] == 12.0  #in this case there is no bearing seat at the bottom chord  
        ...
        x_seat_bottom_bolt_1 = x_support_to_bottom_chord + joist_ends[1] - chord.tab_hole_center_location.Δx
    ...
    if joist_ends[1] == 12.0
        ...
        x_end_compression_diagonal_1_bottom = x_support_to_bottom_chord + Δx_seat_1 + bearing_seat.web_hole_center_locations.Δx[2]
        y_end_compression_diagonal_1_bottom = -depth + Δy_chord_seat_contact - bearing_seat.web_hole_center_locations.Δy[2]
    ...
    if joist_ends[1] == 12.0
        ...
        bottom_start = (joist_ends[1] + x_support_to_bottom_chord, -depth + chord.tab_hole_center_location.Δy)
    ...
    #define location of bottom bolt hole
    x_seat_bottom_bolt_2 = x_support_to_bottom_chord + Δx_seat_2 + bearing_seat.web_hole_center_locations.Δx[1]
    y_seat_bottom_bolt_2 = -depth + Δy_chord_seat_contact - bearing_seat.web_hole_center_locations.Δy[1]
...
function define_bottom_chord_coordinates(joist, chord_section_properties, diagonal_coordinates)
    ...
    #first coordinate
    bottom_chord_coordinates = [(joist.x_support_to_bottom_chord, -joist.depth - chord_section_properties.yc)]
    ...
    bottom_chord_coordinates = [bottom_chord_coordinates; (joist.span_length - joist.x_support_to_bottom_chord, -joist.depth - chord_section_properties.yc)]
