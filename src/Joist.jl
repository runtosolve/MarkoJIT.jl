...
@with_kw struct Inputs
    ...
    chord_splice_dimensions::NamedTuple{(:t, :B, :H, :R), Tuple{Float64, Float64, Float64, Float64}}
    chord_splice_locations::NamedTuple{(:top, :bottom), Tuple{Vector{Float64}, Vector{Float64}}}
...
@with_kw struct ComponentDemands
    ...
    top_chord_splice::Vector{Float64}
    bottom_chord_splice::Vector{Float64}
...
function calculate_chord_splice_DC(element_type, model, chord_splice_location, eRn_chord_splice, chord_demand, joist)
    ...
    joist_ends = MarkoJIT.Geometry.define_joist_ends(joist.span_length, joist.node_spacing)
    ...
    splice_chord_demands = chord_demand[chord_element_index]

    return chord_splice_DC, splice_chord_demands
...
function evaluate_joist_span(design_code, joist_dimensions, chord_dimensions, diagonal_dimensions, bolt_properties, shield_plate_dimensions, diagonal_sections, diagonal_bracing, bearing_seat_dimensions, chord_splice_dimensions,  girder_dimensions, girder_material_properties, bearing_seat_weld_properties, joist_material_properties, top_chord_connections, bottom_chord_connections)
    ...
    chord_section_properties = MarkoJIT.Properties.calculate_chord_section_properties(chord_dimensions)
    ...
    #chord splices 
    # X_top_chord = [coordinates.top_chord[i][1] for i in eachindex(coordinates.top_chord)]
    # X_bottom_chord = [coordinates.bottom_chord[i][1] for i in eachindex(coordinates.bottom_chord)]
    # chord_splice_locations = MarkoJIT.Geometry.define_chord_splice_locations(joist_dimensions.span_length, joist_dimensions.node_spacing, joist_dimensions.bottom_chord_tail_length, X_top_chord, X_bottom_chord)
    chord_splice_locations = MarkoJIT.Geometry.define_chord_splice_locations(joist_dimensions.span_length)
  
    #collect inputs 
   inputs = Inputs(design_code, joist_dimensions, chord_dimensions, diagonal_dimensions, bolt_properties, shield_plate_dimensions, diagonal_sections, diagonal_bracing, bearing_seat_dimensions, chord_splice_dimensions,  chord_splice_locations, girder_dimensions, girder_material_properties, bearing_seat_weld_properties, joist_material_properties, top_chord_connections, bottom_chord_connections)
    ...
    Pcrℓ_diagonal, diagonal_local_buckling = MarkoJIT.Properties.calculate_diagonal_local_buckling_load(diagonal_section_geometry, diagonal_dimensions, joist_material_properties)
    ...
    Pcre_diagonal, diagonal_global_buckling, Lu = Strength.calculate_diagonal_global_buckling(model, diagonal_sections, diagonal_bracing, diagonal_dimensions, diagonal_section_geometry, joist_material_properties)
    ...
    # joist_ends = MarkoJIT.Geometry.define_joist_ends(joist_dimensions.span_length, joist_dimensions.node_spacing)
    # if joist_ends
    # num_bolts = 4
    bearing_seat_chord_connection_strengths = Strength.calculate_bearing_seat_chord_connection_strength(joist_dimensions, bolt_properties, joist_material_properties, design_code, bearing_seat_dimensions, chord_dimensions)
    ...
    bottom_chord_splice_DC, bottom_chord_splice_demand = calculate_chord_splice_DC("bottom chord", model, chord_splice_locations.bottom, eRn_chord_splice, bottom_chord_tension_demand, joist_dimensions)
    top_chord_splice_DC, top_chord_splice_demand = calculate_chord_splice_DC("top chord", model, chord_splice_locations.top, eRn_chord_splice, top_chord_compression_demand, joist_dimensions)


    bearing_seat_weld_demands = [abs.(top_chord_compression_demand[2])./1000, abs.(top_chord_compression_demand[end-1])./1000]  #assume all of top chord force is carried by weld, pinned-pinned condition 
    eRn_bearing_seat_weld = bearing_seat_weld_strength.eRn
    bearing_seat_weld_DC_1 = calculate_demand_to_capacity(bearing_seat_weld_demands[1], eRn_bearing_seat_weld)
    bearing_seat_weld_DC_2 = calculate_demand_to_capacity(bearing_seat_weld_demands[2], eRn_bearing_seat_weld)
    ...
    eRn_bearing_seat_chord_connections = [bearing_seat_chord_connection_strengths[1].eRn, bearing_seat_chord_connection_strengths[2].eRn]
    bearing_seat_chord_connection_DC_1 = calculate_demand_to_capacity(abs.(model.solution.forces[top_chord_elements[2]][1])./1000, bearing_seat_chord_connection_strengths[1].eRn)
    bearing_seat_chord_connection_DC_2 = calculate_demand_to_capacity(abs.(model.solution.forces[top_chord_elements[end-1]][1])./1000, bearing_seat_chord_connection_strengths[2].eRn)
    ...
    demand = ComponentDemands(diagonal_demands, top_chord_compression_demand, bottom_chord_tension_demand, top_chord_splice_demand, bottom_chord_splice_demand, bearing_seat_weld_demands, bearing_seat_compression_field_demands, bearing_seat_chord_connection_demands)


    strength = Strength.Components(diagonal_tensile_strength, diagonal_compressive_strength, chord_compressive_strength, chord_tensile_strength, chord_splice_strength, bearing_seat_weld_strength, bearing_seat_compression_field_strength, bearing_seat_chord_connection_strengths, top_chord_connection_strengths, bottom_chord_connection_strengths, eRn_diagonals, eRn_top_chord_connections, eRn_bottom_chord_connections, eRn_top_chord, eRn_bottom_chord, eRn_chord_splice, eRn_bearing_seat_weld, eRn_bearing_compression_field, eRn_bearing_seat_chord_connections)
    ...
    joist_span = JoistSpan(inputs, properties, joist_end_types, model, strength, Δ_joist, demand, demand_to_capacity, limit_state_loads, joist_strength)
